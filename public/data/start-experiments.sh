    ...
    for n_episodes in ${episodes[@]}; do
      for n_tries in ${tries[@]}; do
        ...
        for mode in ${modes[@]}; do
          ...
          bash start-api.sh "apply-model" $size "horizontal" $round $n_clients -1 $n_episodes $n_tries $learning_rate 1 $mode 1
          bash start-api.sh "run-model" $size "horizontal" $round $n_clients -1 $n_episodes $n_tries $learning_rate 1 $mode 1
        ...
        for par_sz in ${par_size[@]}; do
          ...
          bash start-api.sh "apply-model" $size "vertical" $round $n_clients $par_sz $n_episodes $n_tries $learning_rate 0.8 -1 1
          bash start-api.sh "run-model" $size "vertical" $round $n_clients $par_sz $n_episodes $n_tries $learning_rate 0.8 -1 1
...
# Equality test
bash start-api.sh "apply-model" 15 "horizontal" 5 5 -1 10 100 $learning_rate 0 0 2
bash start-api.sh "run-model" 15 "horizontal" 5 5 -1 10 100 $learning_rate 0 0 2
